...
countData_VTA <- read.csv("VTA_v2_count_ready_for_plot.csv", header = TRUE, 
													stringsAsFactors = FALSE)
...
output$table3 <- renderTable({
  countDataPlot_VTA <- countData_VTA[countData_VTA$mgi_symbol %in% {input$geneName}, ]
  df <- select(countDataPlot_VTA, -c(ensembl_gene_id, region, mgi_symbol, genotype,
                                                    condition, sample))
  df$animal <- as.integer(c(1,2,3,4,1,2,3,4,5,6,1,2,3,4,5,6,1,2,3,4))
  df <- spread(df, key = experiment , value = counts)
  return(df)
})

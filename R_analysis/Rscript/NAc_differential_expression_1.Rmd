---
title: "Differential gene expression visualization - NAc WT_lightvsControl"
output: html_document
---
Comparing the effect of light treatment in wild type animal
...
dds.results <- readRDS("NAc/NAc_WT_lightvsControl.RData")
```
...
hist(dds.results$pvalue, breaks = 20, col ='grey', 
		 main = "WT(control Vs light induced)", xlab = "pvalue", ylab = "Frequency")
```

```{r warning=FALSE}
# load the dataframe with the gene info and remark added
setwd("~/Desktop/brainRNASeq/R_analysis/R_output/results_tables/NAc")
dds.results.table <- read.csv("NAc_WT_lightvsControl.csv", header = TRUE,

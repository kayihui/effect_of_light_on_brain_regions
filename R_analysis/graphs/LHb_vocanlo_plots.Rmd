...
LHb.1 <- read.csv("LHb_WT_lightvsControl.csv", header = TRUE, stringsAsFactors = FALSE)
LHb.2 <- read.csv("LHb_Per1_lightvsControl.csv", header = TRUE, stringsAsFactors = FALSE)
LHb.3 <- read.csv("LHb_WTvsPer1_control.csv", header = TRUE, stringsAsFactors = FALSE)
LHb.4 <- read.csv("LHb_WTvsPer1_light.csv", header = TRUE, stringsAsFactors = FALSE)
LHb.5 <- read.csv("LHb_effectAcrossGenotype.csv", header = TRUE, stringsAsFactors = FALSE)
```

## WT Light vs control
```{r fig.height=7, fig.width=7}
LHb.1.updated <- prepare_data_for_graph(LHb.1)
vocanlo_plot(LHb.1.updated)
```

## Per1 Light vs control
```{r fig.height=7, fig.width=7}
LHb.2.updated <- prepare_data_for_graph(LHb.2)
vocanlo_plot(LHb.2.updated)
```

## WT vs Per1 (control)
```{r fig.height=7, fig.width=7}
LHb.3.updated <- prepare_data_for_graph(LHb.3)
vocanlo_plot(LHb.3.updated)
```

## WT vs Per1 (light)
```{r fig.height=7, fig.width=7}
LHb.4.updated <- prepare_data_for_graph(LHb.4)
vocanlo_plot(LHb.4.updated)
```

## the effect of genotype on light treatment
```{r fig.height=7, fig.width=7}
LHb.5.updated <- prepare_data_for_graph(LHb.5)
vocanlo_plot(LHb.5.updated)
```
